...
class BinaryReaderIR : public BinaryReaderNop {
 public:
  ...
  Result OnFunctionBinaryRead(Index index, const uint8_t *data, Offset, Offset);
...
Result BinaryReaderIR::BeginFunctionBody(Index index, Offset size) {
  ...
  current_func_->wasm_bin_size = size;
  PushLabel(LabelType::Func, &current_func_->exprs);
  return Result::Ok;
}

Result BinaryReaderIR::OnFunctionBinaryRead(Index index, const uint8_t *data,
                                            Offset begin, Offset size) {
  current_func_ = module_->funcs[index];
  current_func_->wasm_bin = (UINT8*) data + begin;
  current_func_->wasm_bin_size = size - begin;
  return Result::Ok;
}
